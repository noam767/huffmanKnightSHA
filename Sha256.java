...
public class Sha256 {
    ...
    //those are the first 32 bits of the fractional parts of the cubic roots of the first 64 prime numbers, in hexadecimal form
    private static final int[] H0 = {
            0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
    };
    //those are the first 32 bits of the fractional parts of the square roots of the first 8 prime numbers, in hexadecimal form
    ...
    public static byte[] hash(byte[] message) {
        ...
        for (int i = 0, n = words.length / 16; i < n; ++i) {
            ...
            System.arraycopy(words, i * 16, W, 0, 16); //16 ראשונים ממערך w נשארים אותו דבר
            for (int t = 16; t < W.length; ++t) {
                W[t] = smallSig1(W[t - 2]) + W[t - 7] + smallSig0(W[t - 15]) + W[t - 16]; //נוסחת נסיגה
            }

            // let TEMP = H
            System.arraycopy(H, 0, TEMP, 0, H.length);

            // operate on TEMP (in some places called TEMP[0] is a, TEMP[1] is b,and so on with c,d,e,f,g,h)
            for (int t = 0; t < W.length; ++t) {
                int t1 = TEMP[7] + bigSig1(TEMP[4]) + ch(TEMP[4], TEMP[5], TEMP[6]) + K[t] + W[t]; //נוסחת נסיגה
            ...
            for (int t = 0; t < H.length; ++t) {
                H[t] += TEMP[t]; //updating the H array values
            }
    ...
    /**
     ...
     @param y a 32-bit integer
     @param z a 32-bit integer
     @return a 32-bit integer that depends on the values of x, y, and z
     */

    private static int ch(int x, int y, int z) {
        return (x & y) | ((~x) & z);
    } //ch stands for choose - if x is zero - then choose from z, else (x is 1) - choose from y. This process for all the length of x
    /**
    ...
    /**
     ...
     @param x a 32-bit integer
     @return a 32-bit integer that depends on the value of x
     */
